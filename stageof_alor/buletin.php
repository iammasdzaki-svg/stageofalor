...
<head>
    ...
    <title>Arsip Buletin - Stasiun Geofisika Alor</title>
...
<body class="bg-gray-50 text-gray-800 font-sans">
    ...
    <header class="site-header hero-buletin">
        <h2 class="site-title font-black uppercase tracking-tighter">Arsip Buletin Geofisika</h2>
        <p class="text-blue-200 text-sm mt-2 italic">Berbagai informasi mengenai Geofisika</p>
    </header>
    ...
    <main class="container mx-auto px-4 py-12">
        <div class="flex flex-col md:flex-row justify-between items-start gap-6 mb-10">
            <div class="border-l-8 border-blue-900 pl-6 w-full">
                <h3 class="text-3xl font-black text-gray-800 uppercase tracking-tighter">Daftar Buletin</h3>
                <p class="text-gray-500 italic text-sm">Informasi, analisis, dan kajian mendalam tentang fenomena bumi</p>
            ...
            <div class="relative w-full md:w-96">
                ...
                <input type="text" id="searchInput" onkeyup="liveSearch()" 
                    placeholder="Ketik kata kunci (Contoh: Januari)..." 
                ...
                <div class="flex gap-3 mt-4 items-center">
                    <div>
                        <select id="filterDay" onchange="toggleCustom(this,'filterDayCustom'); liveSearch()" class="px-3 py-2 rounded-xl bg-white border-2 border-gray-100 text-sm">
                        ...
                        <input id="filterDayCustom" oninput="liveSearch()" placeholder="Masukkan hari" class="px-3 py-2 rounded-xl border-2 border-gray-100 text-sm mt-2 hidden" />
                    </div>

                    <div>
                        <select id="filterMonth" onchange="toggleCustom(this,'filterMonthCustom'); liveSearch()" class="px-3 py-2 rounded-xl bg-white border-2 border-gray-100 text-sm">
                        ...
                        <input id="filterMonthCustom" oninput="liveSearch()" placeholder="Masukkan bulan (nama atau angka)" class="px-3 py-2 rounded-xl border-2 border-gray-100 text-sm mt-2 hidden" />
                    </div>
        ...
        <div id="file-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            if (is_dir($dir)) {
                ...
                if (count($files) > 0) {
                        ...
                        <div class="buletin-card bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden hover:shadow-2xl transition-all duration-500 group" data-filename="<?php echo strtolower($namaFile); ?>" data-filepath="<?php echo $file; ?>">
                            ...
                            <div class="p-8">
                                <div class="flex justify-between items-center mb-2">
                                    <span class="text-[9px] font-black text-blue-600 tracking-widest uppercase">E-Buletin</span>
                    ...
                    echo "<p class='col-span-full text-center text-gray-400 py-20'>Belum ada file PDF di folder file_buletin.</p>";
                }
            } else {
                echo "<p class='col-span-full text-center text-red-400 py-20'>Folder 'file_buletin' tidak ditemukan.</p>";
            }
            ?>
        </div>
        ...
        <div id="noResult" class="hidden text-center py-20">
            ...
            <p class="text-gray-400 font-bold italic">Nama buletin tersebut tidak ditemukan...</p>
        </div>
    ...
    <script>
        ...
        function liveSearch() {
            ...
            const day = (daySel.value === 'custom') ? (document.getElementById('filterDayCustom').value || '').trim() : (daySel.value || '').trim();
            const month = (monthSel.value === 'custom') ? (document.getElementById('filterMonthCustom').value || '').trim().toLowerCase() : (monthSel.value || '').trim().toLowerCase();
            ...
            for (let i = 0; i < cards.length; i++) {
                ...
                if (day) {
                    ...
                    if (!(fname.includes(dn) || fname.includes(day))) match = false;
                }

                if (month) {
                    // month can be numeric (1,01,12) or name (januari)
                    let monthNumeric = month;
                    if (!/^[0-9]+$/.test(month)) {
                        // map name to number
                        for (const [name,num] of Object.entries(monthNames)) {
                            if (month.includes(name)) { monthNumeric = num; break; }
                        }
                    }
                    monthNumeric = (''+monthNumeric).padStart(2,'0');
                    let monthMatch = fname.includes(monthNumeric);
                    for (const name of Object.keys(monthNames)) {
                        if (fname.includes(name)) monthMatch = true;
                    }
                    if (!monthMatch) match = false;
                }
