...
<head>
    ...
    <title>Infografis Seismisitas - Stasiun Geofisika Alor</title>
...
<body class="bg-gray-50 text-gray-800 font-sans">
    ...
    <header class="site-header hero-infografis border-b-4 border-yellow-500">
        <div class="container mx-auto px-4">
            <h2 class="site-title font-black mb-4 uppercase tracking-tighter">Infografis Seismisitas</h2>
            <p class="text-sm opacity-90 max-w-2xl mx-auto font-light">Peta distribusi gempa bumi di wilayah Kabupaten Alor dan sekitarnya</p>
        </div>
    </header>
    ...
    <main class="container mx-auto px-4 py-12">
        ...
        <?php
        $dir = "file_infografis_seismisitas/";
        $allFiles = [];
        if (is_dir($dir)) {
        ...
        <div class="flex flex-col md:flex-row justify-between items-start gap-6 mb-10">
            <div class="border-l-8 border-red-600 pl-6 w-full">
                <h3 class="text-2xl font-black text-gray-900 uppercase tracking-tighter">Filter Seismisitas</h3>
                <p class="text-gray-500 italic text-sm">Cari infografis berdasarkan judul atau tanggal pada nama file.</p>
                
                <div class="flex gap-2 mt-4">
                    <button onclick="toggleInfografisView('grid')" id="infografis-grid-btn" class="px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-red-700 transition active:scale-95">
            ...
            <div class="relative w-full md:w-96">
                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <i class="fas fa-search text-red-900 opacity-50"></i>
                </div>
                <input type="text" id="searchInput" onkeyup="liveSearch()" placeholder="Ketik kata kunci..." class="w-full pl-11 pr-4 py-4 bg-white border-2 border-gray-100 rounded-2xl shadow-sm focus:border-red-600 focus:ring-0 outline-none transition-all text-sm font-medium">
        ...
        <section id="seismisitas" class="mb-20">
            <div class="flex items-center mb-8">
                <div class="w-12 h-12 bg-red-600 rounded-xl flex items-center justify-center text-white text-xl shadow-lg mr-4">
                    <i class="fas fa-wave-square"></i>
                </div>
                <div>
                    <h3 class="text-2xl font-black text-gray-900 uppercase leading-none">Arsip Seismisitas</h3>
            ...
            <div id="file-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <?php if (count($allFiles) > 0): ?>
                    ...
                    <div class="infografis-card bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden hover:shadow-2xl transition-all duration-500 group" data-filename="<?php echo strtolower($namaFile); ?>" data-filepath="<?php echo $file; ?>">
                        <canvas class="infografis-preview w-full h-28 bg-red-50 flex items-center justify-center relative overflow-hidden" style="display: none;"></canvas>
                        <div class="infografis-placeholder h-28 bg-red-50 flex items-center justify-center relative overflow-hidden">
                            <i class="fas fa-file-pdf text-5xl text-red-600 opacity-10 group-hover:scale-125 transition-transform duration-700"></i>
                            <div class="absolute inset-0 bg-gradient-to-t from-white to-transparent opacity-40"></div>
                        </div>
                        <div class="p-8">
                            <div class="flex justify-between items-center mb-2">
                                <span class="text-[9px] font-black text-red-600 tracking-widest uppercase">Seismisitas</span>
                            ...
                            <div class="flex gap-3">
                                <a href="<?php echo $file; ?>" 
                                   ...
                                   class="flex-1 flex items-center justify-center bg-red-600 hover:bg-red-700 text-white py-3 rounded-xl text-[10px] font-black transition-all shadow-lg active:scale-95 uppercase">
                                    <i class="fas fa-eye mr-2 text-sm"></i> LIHAT
                                </a>
                                <a href="<?php echo $file; ?>" 
                                   download 
                                   class="flex-1 flex items-center justify-center bg-yellow-500 hover:bg-yellow-400 text-gray-900 py-3 rounded-xl text-[10px] font-black transition-all shadow-lg active:scale-95 uppercase">
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <p class="col-span-full text-center text-gray-400 py-20">Belum ada file PDF di folder file_infografis_seismisitas.</p>
                <?php endif; ?>
            </div>
